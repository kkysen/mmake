...
import {AllTypedArray, TypedArray} from "../types/TypedArray";
...
const makeShaWebCrypto: MakeSha = function(numBits: number): Hash {
    const digest: (buffer: Buffer) => PromiseLike<ArrayBuffer> =
    ...
    return {
        ...
        hash(data: string | Buffer): Promise<string> {
            ...
            const dataViewOrString = isString(data)
                ...
                : isDataView(data)
                    ? data
                    : new DataView(isArrayBuffer(data) ? data : (data as AllTypedArray).buffer);
